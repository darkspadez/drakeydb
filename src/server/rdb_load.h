...
class RdbLoaderBase {
 protected:
  ...
  // RdbLoader::SetApplyLwwGuard before Load() runs (DflyShardReplica's constructor -- replica.cc).
  // Stays false for every other loader (local RDB file load, plain replica's full sync, a test
  // construction that never calls the setter), matching apply_origin_idx_'s own default story.
...
class RdbLoader : protected RdbLoaderBase {
 public:
  ...
  // class). `on` should be the SAME value the flow's stable-sync executor_ was given
  // (DflyShardReplica's constructor reads it back from executor_->connection_context()->
  // repl_lww_guard rather than recomputing peer_mode_ && IsActiveReplica() && the flag a second
  // time, so this loader and executor_ can never disagree about one link's guard). Leave unset
  // (default false) for a plain (non-peer) replica's loader.
