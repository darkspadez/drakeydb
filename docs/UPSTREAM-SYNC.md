...
| `src/server/string_family.cc` | `SetCmd::RecordJournal` is the single chokepoint every conditional `SET` variant (`SETEX`, `SET ... EX/PX/EXAT/PXAT/NX/XX/KEEPTTL/GET`, memcache flags) journals through, uniformly as plain `SET`; also hosts `OpMSet`'s own per-key LWW self-guard (P4-4) and `GETEX`/`GAT`'s own `PERSIST`/`PEXPIREAT` normalization (a separate call site from generic_family.cc's EXPIRE-family one — easy to miss when auditing one file in isolation). **Hazard:** same as generic_family.cc above — the guard's vocabulary table depends on every conditional `SET` variant landing on `SET`; a new variant that bypasses `SetCmd::RecordJournal` would silently escape the guard |
